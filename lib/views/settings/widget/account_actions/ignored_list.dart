...
class IgnoredList extends StatefulWidget {
  const IgnoredList({Key? key}) : super(key: key);

  @override
  _IgnoredListState createState() => _IgnoredListState();
}

class _IgnoredListState extends State<IgnoredList> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        appBar: PreferredSize(
          ...
          child: defaultAppBar(context, title: "Ignored List"),
        ),
        body: Consumer<SetUpProvider>(builder: (context, provider, child) {
          return Column(
            children: [
              GreyContainer(title: "Accounts You Have Ignored"),
              const SizedBox(height: 20),
              ListView.builder(
                  shrinkWrap: true,
                  itemCount: provider.ignoreList?.length,
                  itemBuilder: (context, item) {
                    return _item(provider.ignoreList![item], addUser: () {
                      provider.ignoreList?.remove(provider.ignoreList?[item]);
                      setState(() {});
                    });
                  })
            ],
  ...
  Widget _item(user, {required Function addUser}) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          TextButton(
              onPressed: () {
                // setUpBloc.add(UnIgnoreAUser(
                //     userId: appState.user["id"], blockedId: user.id));
                _setUpProvider!.unignoreUser(
